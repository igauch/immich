...
export class AssetMediaService extends BaseService {
  ...
  private async create(ownerId: string, dto: AssetMediaCreateDto, file: UploadFile, sidecarFile?: UploadFile) {
    ...
    const asset = await this.assetRepository.create({
      ...
      checksum: file.checksum,
      fileHash: file.fileHash,
      originalPath: encryptedFilePath || file.originalPath,
      ...
      type: mimeTypes.assetType(encryptedFilePath || file.originalPath),
      ...
      encryptionIv: encryptionInfo?.iv || null,
      encryptionSalt: encryptionInfo?.salt || null,
    });
